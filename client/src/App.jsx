    ...
    <div className="bg-[#0B0A08] text-[#F5F1E6] font-body antialiased">
      ...
      <header
        className={`fixed top-0 inset-x-0 z-50 transition-colors duration-300 ${
          scrolled ? "bg-[#0B0A08]/95 backdrop-blur border-b border-white/10" : "bg-transparent"
        }`}
      >
        <div className="max-w-7xl mx-auto px-6 md:px-10 h-20 flex items-center justify-between">
          <a href="#home" className="leading-none">
            <span className="block font-display text-2xl tracking-[0.3em]">TRAX</span>
          ...
          <nav className="hidden lg:flex items-center gap-8">
            {NAV_LINKS.map((l) => (
              <a
                key={l.href}
                href={l.href}
                className="relative text-[11px] tracking-[0.15em] uppercase text-[#F5F1E6]/85 hover:text-[#C6A15B] transition-colors pb-1"
              >
                {l.label}
                {l.href === "#home" && (
      ...
      {/* ---------- FOOTER ---------- */}
      <footer className="bg-[#0B0A08] border-t border-white/10 px-6 md:px-10 py-10">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-[#F5F1E6]/50">
          <p className="tracking-[0.15em]">TRAX MANAGEMENT GROUP © {new Date().getFullYear()}</p>
          <a href={SITE_URL} target="_blank" rel="noopener noreferrer" className="tracking-[0.15em] hover:text-[#C6A15B]">
            TRAXMANAGEMENTGROUP.COM
          </a>
        </div>
      </footer>
    </div>
